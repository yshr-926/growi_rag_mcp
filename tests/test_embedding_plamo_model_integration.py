...
2) Given text input, when embed/encode functions run, then 1024-dimensional L2-normalized embeddings are returned.
...
def test_embed_returns_1024_dimensional_normalized_vector(loaded_model: PlamoEmbeddingModel) -> None:
    """Embedding vectors must be 1024-d float32 arrays that are L2-normalized."""
    ...
    assert vector.shape == (EXPECTED_DIM,), "plamo-embedding-1b must return 1024-d embeddings"
...
def test_encode_query_and_document_are_consistent(loaded_model: PlamoEmbeddingModel) -> None:
    ...
    assert query_vec.shape == (EXPECTED_DIM,), "Query embedding must be 1024-dimensional"
    assert doc_vec.shape == (EXPECTED_DIM,), "Document embedding must be 1024-dimensional"
...
def test_model_reports_expected_embedding_dimension(loaded_model: PlamoEmbeddingModel) -> None:
    """Model metadata should advertise the 1024 expected embedding dimension."""
    assert (
